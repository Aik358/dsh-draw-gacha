...
function score(s) {
  ...
  const length = observed >= 2200 ? 5 : observed >= 1200 ? 3 : observed >= 500 ? 2 : observed >= 160 ? 1 : 0;
  const normal = s.finish === null || s.finish === 'stop' || s.finish === 'tool-calls';
  const letMePenalty = Math.min(1, Math.max(0, c.letMe - (planning + rigor)));
  const total = planning + rigor + length + (normal ? 1 : 0) - letMePenalty;
  if (s.finish === 'error' || s.finish === 'aborted') return observed >= 500 ? { rarity: 'gold', total } : { rarity: 'blue', total };
  if (observed >= 1200 && total >= 7 && normal) return { rarity: 'red', total };
  if (observed >= 500 && total >= 3 && normal) return { rarity: 'gold', total };
  return { rarity: 'blue', total };
}
function result(s) {
  const scored = score(s);
  const observed = s.reasoningChars + s.textChars;
  if (scored.rarity === 'red' && observed >= 1200) return { rarity: 'red', tails: ['red', 'red', 'red'], stars: 6, centerStage: 'slam', finish: s.finish || 'stop', counts: s.counts, reasoningChars: s.reasoningChars, textChars: s.textChars, seed: s.generation, flavorText: '夯力过载，三舱全红！' };
...
function observe(options, chunk) {
  ...
  else if (chunk.type === 'finish') { const r = typeof chunk.reason === 'string' ? chunk.reason : chunk.reason && (chunk.reason.kind || chunk.reason.type); s.finish = ['stop', 'tool-calls', 'max-tokens', 'aborted', 'error'].includes(r) ? r : 'error'; s.result = result(s); s.phase = s.finish === 'aborted' ? 'cancelled' : s.finish === 'error' ? 'error' : 'settled'; }
  s.updatedAt = stamp();
}
